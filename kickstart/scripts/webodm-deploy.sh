...
deploy_webodm_ubuntu() {
  ...
  docker exec webodm-web.service python manage.py shell -c "from nodeodm.models import ProcessingNode; ProcessingNode.objects.update_or_create(hostname='localhost', defaults={'hostname': 'localhost', 'port': 3000, 'label': 'Cluster'})"
